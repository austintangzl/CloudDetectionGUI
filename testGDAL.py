...
if not dataset:
    ...
    if os.path.isfile(os.getcwd()+'\\newimg.tif') is True:
        os.remove(os.getcwd()+'\\newimg.tif')
    outDs = driver.Create(os.getcwd() + '\\newimg.tif', cols, rows, 3, dataset.GetRasterBand(1).DataType)   # Create the new image driver
    if outDs is None:
    ...
    outData = numpy.zeros((rows, cols, 3), numpy.uint16)
    ...
    # for i in range(3):
    #     tempmapband = workingmap.GetRasterBand(i + 1)
    #     tempmapvalues = np.zeros(shape=(xlen, ylen))
    #     for y in range(tempmapband.YSize):
    #         scanline = tempmapband.ReadRaster(xoff=0, yoff=y, xsize=tempmapband.XSize, ysize=1, buf_xsize=band.XSize, buf_ysize=1, buf_type=gdal.GDT_Float32)
    #         fuckingfucktuple = struct.unpack('f' * tempmapband.XSize, scanline)     # Unpack the stupid read line to a stupid tuple
    #         tempmapvalues[..., y] = np.asarray(fuckingfucktuple)    # Convert the stupid tuple to np array
    #     combinedmapvalues[..., i] = tempmapvalues
