...
class CloudDetectionMain(tk.Tk):
    def __init__(self, parent):
        ...
        self.iconbitmap(os.getcwd() + '\Icons\cloud_icon.ico')
        ...
        self.colorSchemeTagVar = tk.StringVar()
        self.colorSchemeVar = tk.StringVar()
        self.upperThreshSliderTagVar = tk.StringVar()
        self.upper_thresh_var = tk.DoubleVar()
    ...
    def initialize(self):
        ...
        imgpath = os.getcwd() + '\Icons\Agilspace_Logo.png'  # Set the image path
        ...
        self.imgPanel.grid(column=3, row=1, columnspan=1, rowspan=7, sticky='NSEW', padx=5, pady=5)

        # Create a container for Secondary options
        self.options2 = tk.Frame(self, pady=3)
        self.options2.grid(column=0, row=8, rowspan=2, columnspan=3, stick='EW')
        self.options2.grid_columnconfigure(0, weight=1)
        self.options2.grid_columnconfigure(1, weight=1)
        self.options2.grid_columnconfigure(2, weight=1)

        # Create Dropdown menu for detection on Color scheme
        colorSchemeTag = tk.Label(self.options2, textvariable=self.colorSchemeTagVar, anchor='ne', fg='black')
        colorSchemeTag.grid(column=0, row=0, columnspan=1, stick='EW')
        self.colorSchemeTagVar.set(u'Detect color scheme:')
        self.colorScheme = ttk.Combobox(self.options2, textvariable=self.colorSchemeVar)
        self.colorScheme['values'] = ('RGB', 'HSL (Dont Use)', 'YCrCb')
        self.colorScheme.current(0)
        self.colorScheme.grid(column=1, row=0, columnspan=2, stick='EW')

        # Create a slider bar for the reflectance upper threshold
        dummyTag = tk.Label(self.options2, textvariable='', anchor='se')  # Dummy label
        dummyTag.grid(column=0, row=1, columnspan=1)
        upperThreshSliderTag = tk.Label(self.options2, textvariable=self.upperThreshSliderTagVar, anchor='ne', fg='black')
        # upperThreshSliderTag.grid(column=0, row=8, columnspan=1, stick='EW')
        upperThreshSliderTag.grid(column=0, row=2, columnspan=1, stick='EW')
        self.upperThreshSliderTagVar.set(u'Upper Reflectance Threshold:')
        self.upperThreshScale = tk.Scale(self.options2, variable=self.upper_thresh_var, orient=tk.HORIZONTAL, from_=0, to=100, resolution=0.5)
        self.upperThreshScale.set(98)
        # self.scale.grid(column=1, row=9, columnspan=2, rowspan=1, sticky='EW')
        self.upperThreshScale.grid(column=1, row=1, columnspan=2, rowspan=2, sticky='EW')

        # Create a slider bar for the reflec_gain
        # self.sliderTagVar = tk.StringVar()
        dummyTag = tk.Label(self.options2, textvariable='', anchor='se')  # Dummy label
        dummyTag.grid(column=0, row=3, columnspan=1)
        sliderTag = tk.Label(self.options2, textvariable=self.sliderTagVar, anchor='se', fg='black', borderwidth=2)
        # sliderTag.grid(column=0, row=9, columnspan=1, stick='EW')
        sliderTag.grid(column=0, row=4, columnspan=1, stick='EW')
        self.sliderTagVar.set(u'Reflectance Gain:')
        # self.reflec_gain_var = tk.DoubleVar()
        self.scale = tk.Scale(self.options2, variable=self.reflec_gain_var, orient=tk.HORIZONTAL, from_=0, to=1, resolution=0.05)
        self.scale.set(0.8)
        # self.scale.grid(column=1, row=9, columnspan=2, rowspan=1, sticky='EW')
        self.scale.grid(column=1, row=3, columnspan=2, rowspan=2, sticky='EW')

        # Create a button to start the cloud detection process
        startCloudDetectionButt = tk.Button(self.options2,
                                            text=u'Start Cloud Detection',
                                            command=self.startCloudDetection)
        # startCloudDetectionButt.grid(column=0, row=10, columnspan=1, sticky='EW')
        startCloudDetectionButt.grid(column=0, row=5, columnspan=1, sticky='EW')

        # Create a Dummy Button
        DummyButt = tk.Button(self.options2,
                              text=u'Dummy',
                              command=self.DummyMsg)
        DummyButt.grid(column=1, row=5, columnspan=1, sticky='EW')

        # Create a button to save images
        saveOutputImagesButt = tk.Button(self.options2,
                                         text=u'Save Output',
                                         command=self.saveOutputImages)
        # saveOutputImagesButt.grid(column=2, row=10, columnspan=1, sticky='EW')
        saveOutputImagesButt.grid(column=2, row=5, columnspan=1, sticky='EW')

        # Create a image panel for the final image
        # imgpath2 = 'C:\CloudDetectionGUI3.6\Icons\Empty.png'
        imgpath2 = os.getcwd() + '\Icons\Empty.png'
        ...
        self.imgPanel2.grid(column=3, row=9, columnspan=1, rowspan=6, sticky='NSEW', padx=5, pady=5)

        # Add a text box for the program output
        # self.outputVar = tk.StringVar()
        self.outputText = tk.Text(self, width=45, height=10)
        self.outputText.grid(column=0, row=11, columnspan=3, rowspan=4, sticky='NSEW')
        ...
        self.outputText.insert(tk.END,
                               'Welcome to the Cloud Detection 1.1 program. \nPlease select the image (GeoTIFF/JPEG/PNG) and KML/KMZ file and run the program.')
        ...
        self.grid_rowconfigure(7, weight=1, minsize=120)
        self.grid_rowconfigure(12, weight=1, minsize=100)
    ...
    def startCloudDetection(self):
        ...
        cloud_map, combinedmapvalues, peak_reflec, outputText, Band1, Band2, Band3 = computeX(dataset, cloud_map, row_len, col_len, self.reflec_gain_var.get(), self.upper_thresh_var.get(), self.colorScheme.get())
        self.outputText.insert(tk.END, outputText)
        self.outputText.see(tk.END)
        self.update()

        cloud_map, outputText = computeY(cloud_map, combinedmapvalues, col_len, self.reflec_gain_var.get(), peak_reflec, self.colorScheme.get(), Band1, Band2, Band3)
        self.outputText.insert(tk.END, outputText)
        self.outputText.see(tk.END)
        self.update()
    ...
    def DummyMsg(self):
        messagebox.showinfo('Dummy', 'You Dummy!')
        messagebox.showinfo('Dummy', 'You BohDoh!')
        messagebox.showinfo('Dummy', 'Press For What!')
        messagebox.showinfo('Dummy', 'Alamak!')
        messagebox.showerror('Danger', 'AWAS!')
